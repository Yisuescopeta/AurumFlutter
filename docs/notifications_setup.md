...
## 6) Admin broadcasts and editable discount template

- Admin panel now uses notifications (not email) for mass campaigns.
- Broadcast type stored in `notifications.type = 'admin_broadcast'`.
- Discount favorites copy is editable in `site_settings`:
