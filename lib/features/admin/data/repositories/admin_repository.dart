...
class AdminRepository {
  ...
  Future<void> upsertProductVariantsAndStock({
    ...
    final rows = variants
        .map(
          (v) => {
            ...
            'stock': max(0, _toInt(v['stock'])),
            'sku_variant': v['sku_variant']?.toString(),
          },
        )
        .toList();
    ...
    try {
      await _supabase.rpc(
        'admin_upsert_product_variants_and_stock',
        params: {
  ...
  Future<void> deleteProductIfNoOrders(String productId) async {
    try {
      await _supabase.rpc(
        'admin_delete_product_if_no_orders',
        params: {
          'p_product_id': productId,
        },
      ).timeout(const Duration(seconds: 20));
  ...
  Future<Map<String, String>> getFavoriteDiscountTemplate() async {
    ...
    final title =
        response?['favorite_discount_title_template']?.toString().trim();
    final body =
        response?['favorite_discount_body_template']?.toString().trim();
    ...
    return {
      ...
      'body': (body == null || body.isEmpty)
          ? '{{product_name}} acaba de entrar en oferta.'
          : body,
    };
  }

  Future<void> saveFavoriteDiscountTemplate({
    ...
    await _supabase.from('site_settings').upsert(
      {
        'id': 'main',
        'favorite_discount_title_template': titleTemplate.trim(),
        'favorite_discount_body_template': bodyTemplate.trim(),
        'notification_updated_at': DateTime.now().toIso8601String(),
        'updated_by': userId,
      },
      onConflict: 'id',
    );
